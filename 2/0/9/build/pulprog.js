...
export function makePulprogText(backendModules, allModules, allowLoneModule) {
    ...
    pp.push(ppShortCodeName, ``, ...shortDescriptions, ``, `;$CLASS=HighRes`, `;$DIM=2D`, `;$TYPE=`, `;$SUBTYPE=`, `;$COMMENT=`, ``, citationText, ``, `#include <Avance.incl>`, `#include <Grad.incl>`, `#include <Delay.incl>`, ``, `#ifdef NUS`, `define list<loopcounter> t1list=<$VCLIST>`, `#endif`, ``, preamblesText, `"l0      = td1/${2 * nbl}"             ; Total number of 13C t1 increments`, `"l1      = 0"                 ; Running counter of 13C t1 increments`, `"l2      = 0"                 ; Counter, even for echo, odd for antiecho`, `"l3      = 0"                 ; Running counter for NS`);
    if (asapMixing) {
