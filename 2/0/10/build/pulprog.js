 ...
 *                                    string when NBL < 2.
 */
export function makePulprogText(backendModules, allModules, allowLoneModule) {
    // Initialisation {{{2
    ...
    mainpp.push(`  4u pl1:f1`, `  p16:gp0`, `  d16`, `  (p1 ph0):f1`, `  4u`, `  p16:gp0*1.37`, `  d16`, `  4u BLKGRAD`, `  d1 st0`, `  4u UNBLKGRAD`);
    ...
    // NUS list {{{4
    mainpp.push(`#ifdef NUS`, `  1m t1list.inc`, `#endif`);
    ...
    // 13C t1 {{{4
    if (d0Present) {
        mainpp.push(``, `#ifdef NUS`, `#else`, `  1m id0   ; 13C t1`, `#endif`);
    }
    // 1H t1 {{{4
    if (d10Present && !cnst38Present) {
        mainpp.push(`#ifdef NUS`, `#else`, `  1m id10    ; 1H t1`, `#endif /* NUS */`);
    }
    ...
    let allPpParamNames = [...new Set([
            preamblePlusMainpp.match(/\bp\d{1,2}\b/g),
            preamblePlusMainpp.match(/\bd\d{1,2}\b/g),
            preamblePlusMainpp.match(/\bl\d{1,2}\b/g),
            preamblePlusMainpp.match(/\bsp\d{1,2}\b/g),
            preamblePlusMainpp.match(/\bpl\d{1,2}\b/g),
            preamblePlusMainpp.match(/\bcpd\d{1,2}\b/g),
    ...
    pp.push(ppShortCodeName, ``, ...shortDescriptions, ``, `;$CLASS=HighRes`, `;$DIM=2D`, `;$TYPE=`, `;$SUBTYPE=`, `;$COMMENT=`, ``, citationText, ``, `#include <Avance.incl>`, `#include <Grad.incl>`, `#include <Delay.incl>`, ``, `#ifdef NUS`, `define list<loopcounter> t1list=<$VCLIST>`, `#endif`, ``, preamblesText, `"l0      = td1/${nbl}"               ; TD1/NBL`, `"l1      = 0"                 ; Running counter for delay / phase incrementation`, `"l2      = 0"                 ; Running counter for NS`);
    if (hasInterleaved) {
    ...
    pp.push(``, ...paramDefns, ``, auProgsStr, `; module identifiers: ${backendModules.join(" ")}`, `; pulse programme created by genesis-v${version}, https://nmr-genesis.co.uk`, `; ${(new Date()).toString()}`);
    return pp.join("\n");
}
// }}}1
