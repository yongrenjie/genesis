    ...
    for (let module of simpleModules) {
        ...
        // Deal with first C13 module (the one with variable INEPT excitation)
            ...
            if (module === "ci13_hsqc_tocsy") {
                ...
                if (c2ModulePresent)
                    trueModules.push("CI_HSQC_TOCSY");
                else
                    trueModules.push("C_HSQC_TOCSY");
            }
...
function updatePulprogText() {
    ...
    let ppText;
    try {
        ppText = makePulprogText(moduleNames, allModules, devModeButton.checked);
    }
    catch (error) {
        ppText = "";
    }
